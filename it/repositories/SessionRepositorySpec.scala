  ...
  "SessionRepository" - {
    ...
    "set" - {
      ...
      "must create new document when given valid UserAnswers" in {
        ...
        val getResult = repository.get(LocalReferenceNumber("ABCD3333333333333").get, EoriNumber("EoriNumber3")).futureValue.value
      ...
      "must fail when attempting to create using an existing id" in {
        ...
        whenReady(setResult.failed) {
          _ mustBe a[MongoWriteException]
        }
      }
    }
    ...
    "remove" - {
      ...
      "must remove document when given a valid LocalReferenceNumber and EoriNumber" in {
        ...
        repository.get(LocalReferenceNumber("ABCD1111111111111").get, EoriNumber("EoriNumber1")).futureValue mustBe defined

        repository.remove(LocalReferenceNumber("ABCD1111111111111").get, EoriNumber("EoriNumber1")).futureValue

        repository.get(LocalReferenceNumber("ABCD1111111111111").get, EoriNumber("EoriNumber1")).futureValue must not be defined
      }
    }
  }
}
