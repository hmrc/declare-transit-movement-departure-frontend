...
import models.reference.{CircumstanceIndicator, CountryCode, PackageType}
import models.{
  CarrierAddress,
  ConsigneeAddress,
  ConsignorAddress,
...
trait JourneyModelGenerators {
  ...
  implicit def arbitrarySafetyAndSecurityPersonalInformation(implicit arbAddress: Arbitrary[Address]): Arbitrary[SafetyAndSecurity.PersonalInformation] =
    Arbitrary {
      ...
      } yield SafetyAndSecurity.PersonalInformation(name, address)
    }
  ...
  implicit lazy val arbitraryPreTaskListDetails: Arbitrary[PreTaskListDetails] =
    Arbitrary {
      ...
      } yield PreTaskListDetails(lrn, procedureType, false) // TODO this needs to be updated when JourneyDomain has been resolved
    }
