...
class AddItemsCommonAddressFormProviderSpec extends StringFieldBehaviours {
  ...
  private val form         = formProvider(countries, name, Index(0))
  ...
  ".AddressLine3" - {
    ...
    behave like fieldWithMaxLength(
      ...
      fieldName,
      maxLength   = maxLength,
      lengthError = FormError(fieldName, lengthKey, Seq(name, 1))
    )

    behave like mandatoryField(
      form,
      fieldName,
      requiredError = FormError(fieldName, requiredKey, Seq(name, 1))
    )

    behave like fieldWithInvalidCharacters(form, fieldName, invalidKey, maxLength, name, 1)
  }
}
