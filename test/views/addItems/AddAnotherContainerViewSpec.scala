...
class AddAnotherContainerViewSpec extends SingleViewSpec("addItems/containers/addAnotherContainer.njk") {
  ...
  "must display the maxLimitReached text when reached maximum containers limit" in {
    val baseJson =
      Json.obj(
  ...
  "must not display the maxLimitReached text when below maximum number of containers limit" in {
    val baseJson =
      Json.obj(
  ...
  "must display the add another container Yes/No radio when below maximum number of containers limit" in {
    val baseJson =
      Json.obj(
        "allowMoreContainers" -> true
      )
  ...
  "must not display the add another container Yes/No radio when reached maximum number of containers limit" in {
    val baseJson =
      Json.obj(
        "allowMoreContainers" -> false
      )
