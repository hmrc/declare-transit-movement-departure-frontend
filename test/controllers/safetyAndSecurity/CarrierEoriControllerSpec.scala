...
import pages.safetyAndSecurity.CarrierEoriPage
...
class CarrierEoriControllerSpec extends SpecBase with AppWithDefaultMockFixtures with MockitoSugar with NunjucksSupport with JsonMatchers {

  private val formProvider = new CarrierEoriFormProvider()
  private val form         = formProvider()
  private val template     = "safetyAndSecurity/carrierEori.njk"

  lazy val carrierEoriRoute = routes.CarrierEoriController.onPageLoad(lrn, NormalMode).url
  ...
  "CarrierEori Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = emptyUserAnswers.set(CarrierEoriPage, "answer").success.value
    ...
    "must return a Bad Request and errors when invalid data is submitted" in {
      ...
      val request                                = FakeRequest(POST, carrierEoriRoute).withFormUrlEncodedBody(("value", ""))
