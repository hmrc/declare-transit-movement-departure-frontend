...
import pages.addItems.traderSecurityDetails.AddSecurityConsigneesEoriPage
...
class AddSecurityConsigneesEoriControllerSpec extends SpecBase with AppWithDefaultMockFixtures with MockitoSugar with NunjucksSupport with JsonMatchers {
  ...
  "AddSecurityConsigneesEori Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      val request                                = FakeRequest(GET, addSecurityConsigneesEoriRoute)
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(lrn, eoriNumber).set(AddSecurityConsigneesEoriPage(index), true).success.value
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      val request =
        FakeRequest(POST, addSecurityConsigneesEoriRoute)
    ...
    "must return a Bad Request and errors when invalid data is submitted" in {
      ...
      val request                                = FakeRequest(POST, addSecurityConsigneesEoriRoute).withFormUrlEncodedBody(("value", ""))
