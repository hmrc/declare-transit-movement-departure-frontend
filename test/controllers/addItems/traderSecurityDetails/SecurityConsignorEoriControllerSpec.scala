...
class SecurityConsignorEoriControllerSpec extends SpecBase with MockNunjucksRendererApp with MockitoSugar with NunjucksSupport with JsonMatchers {

  def onwardRoute = Call("GET", "/foo")

  private val formProvider = new EoriNumberFormProvider()
  private val form         = formProvider("securityConsignorEori")
  ...
  "SecurityConsignorEori Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      val request        = FakeRequest(GET, securityConsignorEoriRoute)
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = emptyUserAnswers.set(SecurityConsignorEoriPage(index), "answer").success.value
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      val request =
        FakeRequest(POST, securityConsignorEoriRoute)
    ...
    "must return a Bad Request and errors when invalid data is submitted" in {
      ...
      val request        = FakeRequest(POST, securityConsignorEoriRoute).withFormUrlEncodedBody(("value", ""))
