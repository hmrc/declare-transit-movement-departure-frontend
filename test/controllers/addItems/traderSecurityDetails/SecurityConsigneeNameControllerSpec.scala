...
import forms.addItems.traderSecurityDetails.SecurityConsigneeNameFormProvider
...
class SecurityConsigneeNameControllerSpec extends SpecBase with AppWithDefaultMockFixtures with MockitoSugar with NunjucksSupport with JsonMatchers {
  ...
  "SecurityConsigneeName Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      val request                                = FakeRequest(GET, securityConsigneeNameRoute)
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = emptyUserAnswers.set(SecurityConsigneeNamePage(index), "answer").success.value
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      val request =
        FakeRequest(POST, securityConsigneeNameRoute)
    ...
    "must return a Bad Request and errors when invalid data is submitted" in {
      ...
      val request                                = FakeRequest(POST, securityConsigneeNameRoute).withFormUrlEncodedBody(("value", ""))
