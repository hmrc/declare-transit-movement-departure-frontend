...
class PreLodgeDeclarationController @Inject() (
  ...
  formProvider: PreLodgeDeclarationFormProvider,
  ...
  def onPageLoad(lrn: LocalReferenceNumber, mode: Mode): Action[AnyContent] = (identify andThen getData(lrn) andThen requireData).async {
    implicit request =>
      val preparedForm = request.userAnswers.get(PreLodgeDeclarationPage) match {
        case None        => form
    ...
    implicit request =>
      form
        ...
        .fold(
          ...
          value =>
            for {
              updatedAnswers <- Future.fromTry(request.userAnswers.set(PreLodgeDeclarationPage, value))
              _              <- sessionRepository.set(updatedAnswers)
            } yield Redirect(navigator.nextPage(PreLodgeDeclarationPage, mode, updatedAnswers))
        )
  }
}
