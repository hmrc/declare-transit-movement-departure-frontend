...
class RepresentativeNameController @Inject()(
  ...
  def onPageLoad(lrn: LocalReferenceNumber, mode: Mode): Action[AnyContent] = (identify andThen getData(lrn) andThen requireData).async {
    implicit request =>
      val preparedForm = request.userAnswers.get(RepresentativeNamePage) match {
        case None        => form
      ...
      renderer.render("representativeName.njk", json).map(Ok(_))
  }
      ...
      form
        ...
        .fold(
          formWithErrors => {
            ...
            renderer.render("representativeName.njk", json).map(BadRequest(_))
          },
          value =>
            for {
              updatedAnswers <- Future.fromTry(request.userAnswers.set(RepresentativeNamePage, value))
              _              <- sessionRepository.set(updatedAnswers)
            } yield Redirect(navigator.nextPage(RepresentativeNamePage, mode, updatedAnswers))
        )
  }
}
