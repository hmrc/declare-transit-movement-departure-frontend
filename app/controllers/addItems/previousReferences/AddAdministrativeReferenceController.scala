...
class AddAdministrativeReferenceController @Inject()(
  ...
  def onPageLoad(lrn: LocalReferenceNumber, itemIndex: Index, mode: Mode): Action[AnyContent] =
    (identify
      ...
      implicit request =>
        val preparedForm = request.userAnswers.get(addItems.AddAdministrativeReferencePage(itemIndex)) match {
          case None        => form
      ...
      implicit request =>
        form
          ...
          .fold(
            ...
            value =>
              for {
                updatedAnswers <- Future.fromTry(request.userAnswers.set(addItems.AddAdministrativeReferencePage(itemIndex), value))
                _              <- sessionRepository.set(updatedAnswers)
              } yield Redirect(navigator.nextPage(addItems.AddAdministrativeReferencePage(itemIndex), mode, updatedAnswers))
          )
    }
}
