...
class IsConsignorEoriKnownController @Inject() (
  ...
  def onPageLoad(lrn: LocalReferenceNumber, mode: Mode): Action[AnyContent] = (identify andThen getData(lrn) andThen requireData).async {
    implicit request =>
      val preparedForm = request.userAnswers.get(IsConsignorEoriKnownPage) match {
        case None        => form
      ...
      renderer.render("isConsignorEoriKnown.njk", json).map(Ok(_))
  }
      ...
      form
        ...
        .fold(
          formWithErrors => {
            ...
            renderer.render("isConsignorEoriKnown.njk", json).map(BadRequest(_))
          },
          value =>
            for {
