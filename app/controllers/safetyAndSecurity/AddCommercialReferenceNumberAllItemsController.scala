...
class AddCommercialReferenceNumberAllItemsController @Inject() (
  ...
  private val form     = formProvider("addCommercialReferenceNumberAllItems")
  ...
  def onPageLoad(lrn: LocalReferenceNumber, mode: Mode): Action[AnyContent] =
    (identify
      ...
      implicit request =>
        val preparedForm = request.userAnswers.get(AddCommercialReferenceNumberAllItemsPage) match {
          case None        => form
      ...
      implicit request =>
        form
          ...
          .fold(
            ...
            value =>
              for {
                updatedAnswers <- Future.fromTry(request.userAnswers.set(AddCommercialReferenceNumberAllItemsPage, value))
                _              <- sessionRepository.set(updatedAnswers)
              } yield Redirect(navigator.nextPage(AddCommercialReferenceNumberAllItemsPage, mode, updatedAnswers))
          )
    }
}
