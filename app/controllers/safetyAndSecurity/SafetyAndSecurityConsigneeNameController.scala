...
class SafetyAndSecurityConsigneeNameController @Inject() (
  ...
  private val form     = formProvider("safetyAndSecurityConsigneeName")
  private val template = "safetyAndSecurity/safetyAndSecurityConsigneeName.njk"
  ...
  def onPageLoad(lrn: LocalReferenceNumber, mode: Mode): Action[AnyContent] =
    (identify
      ...
      implicit request =>
        val preparedForm = request.userAnswers.get(SafetyAndSecurityConsigneeNamePage) match {
          case None        => form
      ...
      implicit request =>
        form
          ...
          .fold(
            ...
            value =>
              for {
                updatedAnswers <- Future.fromTry(request.userAnswers.set(SafetyAndSecurityConsigneeNamePage, value))
                _              <- sessionRepository.set(updatedAnswers)
              } yield Redirect(navigator.nextPage(SafetyAndSecurityConsigneeNamePage, mode, updatedAnswers))
          )
    }
}
